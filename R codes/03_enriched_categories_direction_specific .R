...
for (cv in cultivars) {
  ...
  # This column is what every downstream step (network, barplot) will use,
  # so all outputs stay consistent with each other and with sig_cutoff/lfc_cutoff.
...
#COEXPRESSION NETWORK, PER CULTIVAR (heatmap removed -- the network is kept
#because the module detection section further below depends on it)
...
for (cv in cultivars) {
  ...
  cat(cv, "-- significant genes:", nrow(sig_cv), "\n")
  
  if (nrow(sig_cv) == 0) {
    cat(cv, "-- no significant genes, skipping\n")
    next
  }
  ...
  # Quitar genes con varianza cero (si no, scale() genera NaN)
  row_var <- apply(mat_cv, 1, var)
  if (any(row_var == 0)) {
    cat(cv, "-- removing", sum(row_var == 0), "zero-variance genes\n")
  }
  mat_cv <- mat_cv[row_var > 0, , drop = FALSE]
  sig_cv <- sig_cv[sig_cv$gene %in% rownames(mat_cv), ]
  
  if (nrow(mat_cv) == 0) {
    cat(cv, "-- no genes left after filtering zero variance, skipping\n")
    next
  }
  
  # --- COEXPRESSION NETWORK (no heatmap) ---
  ...
  tryCatch({
    ...
    plot(g_cv, vertex.label.cex = 0.5, vertex.size = 8,
         main = paste("Coexpression network -", title_cv))
    dev.off()
    cat(cv, "-- coexpression network generated successfully\n")
  }, error = function(e) {
    while (!is.null(dev.list())) dev.off()
    message(cv, " -- ERROR generating network: ", conditionMessage(e))
  })
...
# CR95, significative genes , annotaded to this GO term -- ordered by padj
